...
void transpose8(unsigned char A[8], unsigned char B[8]) {
   ...
   B[7*n]=x>>24;  B[6*n]=x>>16;  B[5*n]=x>>8;  B[4*n]=x; 
   B[3*n]=y>>24;  B[2*n]=y>>16;  B[n]=y>>8;  B[0]=y; 
}
...
void setPattern(int rowPattern, int colPattern) {
   if (rotation == 0) writeToLED(~rowPattern, reverseBits(colPattern));
   else if (rotation == 90) writeToLED(~colPattern, rowPattern);
   else if (rotation == 180) writeToLED(~reverseBits(rowPattern),colPattern);
   else if (rotation == 270) writeToLED(~reverseBits(colPattern),reverseBits(rowPattern));
   else if (rotation == 360) writeToLED(~rowPattern, colPattern);
   else {
     fprintf(stderr, "Invalid rotation: %d", rotation);
     exit(1);
   }
}
...
char updByte1[8] = { 0x7E, 0x81, 0xA5, 0x81, 0xA5, 0x99, 0x81, 0x7E};
char updByte2[8] = { 0x7E, 0x81, 0xA5, 0x81, 0xA5, 0x99, 0x81, 0x7E};
...
int main (int argc, char **argv) {
    ...
    int rflag = 0, sflag = 0;
    char *ropt = 0, *sopt = 0;
    static char usage[] = "Usage: %s -r rotation -s string\n";
    ...
    if ((optind) != argc) {
       ...
       fprintf(stderr, "Err: %s: missing -r option\n", argv[0]);
       fprintf(stderr, usage, argv[0]);
       exit(1);
    } else if (err) {
       fprintf(stderr, usage, argv[0]);
  ...
  delay(500);
  for (;;) {
    for (x = 0; x < strlen(sopt); x++) {
      y1 = (unsigned char) sopt[x];
      y2 = (unsigned char) sopt[x+1];
      transpose8(cp437_font[y1], updByte1);
      transpose8(cp437_font[y2], updByte2);
      scrollText();
    }
  }
