...
void
moveCursor (hd44780 * header, cursorMovement movement)
{
  if (!header)
    return;
...
void
printString (hd44780 * header, char *string)
{
...
void
scrollDisplay (hd44780 * header, displayScroll scroll)
{
  if (!header)
    return;
...
void
cursorControl (hd44780 * header, int state)
{
  if (!header)
    return;
  if (state)
...
void
cursorBlink (hd44780 * header, int state)
{
  if (!header)
    return;
  if (state)
...
void
printInt32 (hd44780 * header, int val)
{
  if (!header)
    return;
...
void
defineCGChars (hd44780 * header, char array[8][8])
{
  char CGbyte;
  int i, j;
  for (i = 0; i < 8; i++)
