...
# Updated by @DGL
# 
#    Added Twitter /twit option
#
#    Different time/date /time /date date time datetime /datetime
#    options and changed locale to dd/mm/yyyy format from mm/dd/yyyy
#
########################################################################
...
#get Twitter OAuth credentials for Tweepy
cur.execute("SELECT consumer_key, consumer_secret, access_token, access_token_secret FROM Oauth where twitter_id = 'MyTweetingArduino';")
...
for row in cur.fetchall() :
    ...
    Akey = row[2]
    Asec = row[3]
# We could hard code credentials here, but I've got them in MySQL
...
def GetSomeText():
    ...
    if st=="/twit":
        ...
        for status in mymentions:
          st=st + status.text + " by @" + status.user.screen_name
       
    # See if the input was either 'time' or 'date'
    # Display time and date if they were requested.
    if st=="/time":
        st = 'The time is ' + time.strftime("%H:%M")
    elif st=="time":
        st = time.strftime("%X")
    elif st=="/date":
        st = 'The date is ' + time.strftime("%a, %d %b %Y")
    elif st=="date":
        st = time.strftime("%x")
    elif st=="/datetime":
        st = time.strftime("%c")
    elif st=="datetime":
        st = time.strftime("%X %x")
